...
@scenario('features/Like_Recipe.feature', 'like a recipe')
def test_like_a_recipe(app):
    pass

@scenario('features/Like_Recipe.feature', 'like a recipe which has been deleted')
def test_like_a_recipe_which_has_been_deleted(app):
    pass

@scenario('features/Like_Recipe.feature', 'like a recipe without logging in')
def test_like_a_recipe_without_logging_in(app):
    pass
...
@given('I did not liked a recipe before', target_fixture="recipe_id")
def i_did_not_liked_a_recipe_before(postgresql, user_id):
    cur = postgresql.cursor()
...
@when('I like this recipe', target_fixture='res')
def i_like_this_recipe(user_id, recipe_id):
    res = like_recipe(recipe_id, user_id)
    return res

@then('the recipe join my list of liked recipes')
def the_recipe_join_my_list_of_liked_recipes(user_id, recipe_id):
    user_like = did_user_like(recipe_id, user_id)
    assert user_like is True
