...
Scenario: Log in to an existing account with username & password (Normal Flow)
...
Scenario: Log in to an existing account with email & password (Alternate Flow)
...
Given the user is not logged into the system
And an account by the name "acc1" does not exist within the system
When requesting to log in to account called "acc1" with password "123"
